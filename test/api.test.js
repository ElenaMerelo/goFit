...
describe("GET /api/workout/discipline/:discipline", () => {
  it("Returns 200", async function () {
    const response = await request(app).get(
      "/api/workout/discipline/kickboxing"
    );
...
describe("GET /api/workout/muscle_group/:muscle_group", () => {
  it("Returns 200", async function () {
    const response = await request(app).get(
      "/api/workout/muscle_group/total_body"
    );
    expect(response.status).to.equal(200);
    expect(response.body.length).to.equal(1);
  });
  ...
  it("Returns 400", async function () {
    const response = await request(app).get(
      "/api/workout/muscle_group/invalid"
    );
    expect(response.status).to.equal(400);
  });
});
...
describe("GET /api/workout/intensity/:intensity", () => {
  it("Returns 200", async function () {
    const response = await request(app).get("/api/workout/intensity/low");
    expect(response.status).to.equal(200);
  ...
  it("Returns 400", async function () {
    const response = await request(app).get("/api/workout/intensity/invalid");
    expect(response.status).to.equal(400);
  });
});
